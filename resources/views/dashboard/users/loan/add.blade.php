...
<x-dashboard.layout>
    <div class="mx-auto max-w-screen-2xl">
        <div class="mb-6">
            <a href="{{ route('dashboard.loan') }}" class="text-slate-400 hover:text-slate-500 mb-4 inline-block">
                <i class="fa-solid fa-arrow-left"></i>
                Back
            </a>
            <h2 class="text-3xl font-bold text-slate-950">
                Form Layout Add Loan
            </h2>
        </div>
        ...
        <div class="flex flex-col gap-9">
            <div class="rounded-sm border border-slate-300 bg-white shadow-lg">
                <form class="py-4 px-6 flex flex-col gap-4" method="POST"
                    action="{{ route('dashboard.loan.add.post.update') }}">
                    @csrf
